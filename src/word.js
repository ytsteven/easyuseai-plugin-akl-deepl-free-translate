...
async function translate(
  ...
  try {
    const url =
      "https://dict.deepl.com/english-chinese/search?ajax=1&source=english&onlyDictEntries=1&translator=dnsof7h3k2lgh3gda&kind=full&eventkind=keyup&forleftside=true&il=zh";
      ...
      if (translation_lines) {
        ...
        for (let i = 0; i < patsArr.length; i++) {
          ...
          if (patsArr[i].part === tmpName) {
            ...
            newArr.push({
              part: tmpName,
              means: JSON.parse(JSON.stringify(means)),
            });
            means = [];
            tmpName = patsArr[i].part;
            means.push(patsArr[i].means[0]);
